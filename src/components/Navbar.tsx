...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
      scrolled ? "bg-dark-purple/90 backdrop-blur-lg shadow-lg" : "bg-transparent"
    }`}>
      <div className="container mx-auto px-4 py-4">
            ...
            <Link to="/" className="mr-6">
              <div className="flex items-center gap-3">
                <img 
                  ...
                  src="/lovable-uploads/5fbc4b30-68bb-4412-8a7b-bf9e138ebc78.png" 
          ...
          <div className="flex items-center space-x-3">
            <form onSubmit={handleSearch} className="hidden md:flex relative">
              <input 
                type="text" 
                placeholder="Search..." 
                className="bg-white/5 backdrop-blur-sm border border-white/10 text-white px-4 py-2 rounded-full focus:outline-none focus:border-primary-purple w-[200px]" 
              />
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-white/10 mt-3 animate-fade-in">
            <div className="flex flex-col space-y-4">
              ...
              <form onSubmit={handleSearch} className="relative mt-2">
                <input 
                  type="text" 
                  placeholder="Search..." 
                  className="w-full bg-white/5 backdrop-blur-sm border border-white/10 text-white px-4 py-2 rounded-full focus:outline-none focus:border-primary-purple" 
                />
                <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2">
                  <Search className="h-5 w-5 text-white/50" />
                </button>
              ...
              {!user && (
                <div className="flex space-x-2 mt-2">
                  <Link to="/auth/sign-in" className="flex-1">
                    <Button 
                      variant="outline" 
                      className="w-full border-primary-purple/30 text-primary-purple hover:bg-primary-purple/10"
                    >
                      Sign In
                    </Button>
                  </Link>
