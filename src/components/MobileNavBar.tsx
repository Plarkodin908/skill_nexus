  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50">
      ...
      <div className="flex justify-around items-center bg-black/80 border-t border-white/10 p-2 px-[15px] py-[4px]">
        <Link to="/" className="p-2 text-white hover:text-white flex flex-col items-center">
          <Home className="h-5 w-5" strokeWidth={3} />
          <span className="text-xs mt-1">Home</span>
        </Link>
        <Link to="/marketplace" className="p-2 text-white hover:text-white flex flex-col items-center">
          <ShoppingBag className="h-5 w-5" strokeWidth={3} />
          <span className="text-xs mt-1">Market</span>
        </Link>
        <Button
          onClick={toggleMenu}
          className="rounded-full bg-black text-white p-3 -mt-5 shadow-lg hover:bg-black/90 my-0 py-[22px]"
        >
          <Menu strokeWidth={3} className="h-5 w-5 my-[5px] px-[2px] mx-[5px] py-0" />
        </Button>
        <Link to="/profile" className="p-2 text-white hover:text-white flex flex-col items-center">
          <User className="h-5 w-5" strokeWidth={3} />
          <span className="text-xs mt-1">Profile</span>
        </Link>
        <Link to="/pricing" className="p-2 text-white hover:text-white flex flex-col items-center">
